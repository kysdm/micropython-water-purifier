...
# 偏移循环位置（防烧屏）：四周均摊——横向 x 取 -2~+2（左右各 2 档）、纵向 y 取 -1~+1（上下各 1 档），
# 共 5×3=15 个位置，蛇形扫过网格并从开机初始偏移 (0,0) 起笔，相邻两步只挪 1px；5 分钟/步 → 一轮 75 分钟。
# 注：正文最小基准坐标为 x=2 / y=1，负向偏移后仍 ≥ 0，正文不被裁剪（仅边框随偏移方向在四边轮流被裁 1~2px）。
ORBIT_POSITIONS = [
    (0, 0), (1, 0), (2, 0),
    (2, -1), (1, -1), (0, -1), (-1, -1), (-2, -1),
    (-2, 0), (-1, 0),
...
def _draw_static_layout():
    ...
    # 边框（偏移后对应方向的边框会被裁剪 1~2 像素，属预期：四边随偏移方向轮流）
...
async def orbit_task():
    """像素偏移任务（防烧屏，仅 OLED 需要）：周期切换偏移位置并重绘，向四周均摊固定元素的像素负载"""
