...
- 屏幕双选（Web 系统配置页面（/system）配置，存于 config.json，不受 OTA 升级影响；未配置时自动识别：I2C 扫描到 OLED 即用 OLED，否则用 TFT）：0.96 寸 SSD1306 OLED（I2C）或 1.8 寸 ST7735 TFT（SPI，128×160）
...
> TFT 模块（1.8 寸）常见的 8 脚丝印为 `SCL`/`SDA`，这是模块厂家的习惯命名——`SCL` 即 SPI 时钟 SCK，`SDA` 即 SPI 数据 MOSI（屏幕只写不读，故无 MISO）。与代码 `TFT_PINS`（[pins.py](pins.py)）的对应关系：
...
| 条件 | 动作 |
|---|---|
| 低压开关 = 1（缺水） | 停止制水（含停止注水），停止强制冲洗任务，等待进水恢复（红色灯 + "缺水"） |
| 高压开关 = 0 且未在制水 | **开始制水**：开进水阀/增压泵，关压力桶出水阀，紫色灯 |
| 高压开关 = 0 且正在制水 | **按纯水TDS注水**（带防抖）：纯水 TDS ≤ 注水TDS（实时或最近可信值）→ 打开压力桶进水阀并行注水；TDS 超过阈值+3ppm 且持续 3 秒 → 关闭桶阀只直供；阈值附近波动不动作 |
| 高压开关 = 1 且正在制水 | **水龙头关闭**：关闭全部阀门/泵（含停止注水），启动强制冲洗任务 + 纯水泡膜倒计时 |
...
stateDiagram-v2
    [*] --> 空闲: 开机初始化（含 18 秒开机冲洗）
    空闲 --> 制水: 高压开关 = 0（用户取水）
    制水 --> 空闲: 高压开关 = 1（用户关水，含停止注水）
    制水 --> 缺水: 低压开关 = 1（进水不足）
    缺水 --> 空闲: 进水恢复
    空闲 --> 冲洗: 停机后自动启动（累计运行达标）
    冲洗 --> 空闲: 冲洗完成（约 9 秒）
...
> 说明：`冲洗` 与 `洗膜` 均在"水龙头关闭停机"后由后台任务并发启动；`冲洗` 仅在累计制水运行时间达标时执行，否则直接跳过；屏幕上的"超时/完成"是洗膜结束时的显示状态，之后回到空闲。
>
> 注水不设独立"注满"判断：压力桶仅在制水期间（纯水 TDS 达标时）被注水，停机即停止注水，桶未满的部分下次制水继续补充；注水 TDS 判断使用实时值，读取失败时回退到最近 15 秒内的可信值，无可信值则不注水。
...
- **像素偏移（仅 OLED）**：画面每 5 分钟在横向 ±2px、纵向 ±1px 范围内**向四周**循环平移（15 个位置，一轮约 75 分钟），均摊固定元素的像素负载；TFT 为 LCD 无烧屏问题，自动跳过；
...
**注意**：画面偏移 1~2px 时，偏移方向那一侧的边框会被裁剪（上下左右轮流出现），属正常现象。
...
|---|---|
| `/` | 主菜单 |
| `/status` | 传感器状态、滤芯使用天数、**重置滤芯**、倒计时/洗膜TDS/注水TDS/泡膜超时/强制冲洗时间设置（GET 查看，POST 修改） |
...
| `display_type` | 自动识别 | oled/tft | 屏幕类型；Web 系统配置页面（/system）选择，**重启生效**；未配置时自动识别（I2C 扫描 OLED）；OTA 升级不覆盖该配置 |
...
设备支持**文件级 OTA**（通过 WiFi 更新 .py 代码文件，不刷 MicroPython 固件）。
...
- 只更新 `.py` 代码文件，**不更新 MicroPython 固件**；仅下载与本地哈希不一致的变更文件；
- 下载中断或校验失败会自动中止并保留旧文件；
- 升级完成后**不会自动重启**，请在页面点**重启设备**生效（重启期间请勿断电）；
- `ota_url` 也可直接在设备 `config.json` 中配置（空字符串 = 未启用）。
...
| 屏幕出现轻微残影/边框缺失 | 防烧屏像素偏移导致 1~2px 裁剪（缺口随偏移方向在四边轮流出现），属正常 |
...
本项目（`micropython-water-purifier`）基于 **GNU GPL v3** 协议开源（见 [LICENSE](LICENSE)）：可自由使用、修改、分发，但衍生作品必须同样以 GPL v3（或兼容协议）开源。字体数据为独立第三方组件，按各自许可分发（见下表，不改变项目整体 GPL v3 授权）。
