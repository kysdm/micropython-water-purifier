...
| 条件 | 动作 |
|---|---|
| 低压开关 = 1（缺水） | 停止制水（直供/注水均停止），停止强制冲洗任务，等待进水恢复（红色灯 + "缺水"） |
| 高压开关 = 0 且未在制水 | **开始制水（直供）**：开进水阀/增压泵，关压力桶出水阀，**不开压力桶进水阀**（避免两路分流影响水龙头水流），紫色灯 |
| 高压开关 = 1 且正在直供 | **水龙头关闭**：纯水 TDS ≤ 注水TDS 时切换为**注水模式**（泵与进水阀保持开启，打开压力桶进水阀为桶注水）；**不达标则跳过注水直接停机**（进入强制冲洗 + 纯水泡膜流程） |
| 高压开关 = 1 且正在注水 | **压力桶注满**：关闭全部阀门/泵，启动强制冲洗任务 + 纯水泡膜倒计时 |
...
stateDiagram-v2
    [*] --> 空闲: 开机初始化（含 18 秒开机冲洗）
    空闲 --> 制水: 高压开关 = 0（用户取水，直供）
    制水 --> 注水: 高压开关 = 1（用户关水，开桶阀注水）
    注水 --> 空闲: 压力桶注满（高压开关再次 = 1）
    制水 --> 缺水: 低压开关 = 1（进水不足）
    注水 --> 缺水: 低压开关 = 1（进水不足）
    缺水 --> 空闲: 进水恢复
    空闲 --> 冲洗: 注满停机后自动启动（累计运行达标）
    冲洗 --> 空闲: 冲洗完成（约 9 秒）
...
> 说明：`冲洗` 与 `洗膜` 均在"压力桶注满停机"后由后台任务并发启动；`冲洗` 仅在累计制水运行时间达标时执行，否则直接跳过；屏幕上的"超时/完成"是洗膜结束时的显示状态，之后回到空闲。
>
> 注水期间若用户重新打开水龙头（高压开关 = 0），由于高压开关只反映总管压力、无法区分"桶未满"与"龙头开启"，固件会保持注水模式直到注满；纯水会优先流向水龙头，属硬件限制下的正常表现。
